...
images = sorted([os.path.join(args.images_dir, f) for f in os.listdir(args.images_dir)])
start = 0
n_images = len(images)
while start < n_images:
    ...
    for index in range(start, end):
        ...
        cv2.imwrite(os.path.join(args.output_dir, os.path.basename(images[index])), Iw)
        # Iw[:,:,0] = 0
        # Iw[:,:,2] = 0
