...
def predict_one_video3(net, Imean, obj_images, hand_images, flow_x_images, flow_y_images):
    crop_size = 224
    num_stack_frames = 10
    mean = 128.0
    action_prob = None
    ...
    for (flow_x_img, flow_y_img, hand_img) in zip(flow_x_images, flow_y_images, hand_images):
        #print flow_x_img, flow_y_img
        Ix = cv2.imread(flow_x_img)
        Iy = cv2.imread(flow_y_img)
        ...
        Ix = Ix[y:y + crop_size, x:x + crop_size, :]
        Iy = Iy[y:y + crop_size, x:x + crop_size, :]
        Ihand = Ihand[y:y + crop_size, x:x + crop_size, :]


        #cv2.imshow('Ix', Ix)
        #cv2.imshow('Iy', Iy)
        #cv2.imshow('Ihand', Ihand)
...
if __name__ == '__main__':
    ...
    with open(args.test_file, 'r') as fr:
        for line in fr.readlines():
            ...
            labels = [int(tokens[3]), int(tokens[4]), int(tokens[5])]
    ...
    for i, video_folder in enumerate(test_videos):
        ...
        score = predict_one_video3(net, Imean, obj_images, hand_images, flow_x_images, flow_y_images)
