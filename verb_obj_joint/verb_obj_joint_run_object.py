...
def predict_one_video3(net, Imean, obj_images, flow_x_images, flow_y_images):
    ...
    for (flow_x_img, flow_y_img) in zip(flow_x_images, flow_y_images):
        ...
        if len(Iq_x) == num_stack_frames:
            ...
            action_score = ret['object_score'].ravel()
...
if __name__ == '__main__':
    ...
    #parser.add_argument('object_ids_file', help = 'Object id file')
    ...
    output_dir = '{}_obj_output'.format(dataset)
    ...
    #object_types = dict()
    #with open(args.object_ids_file, 'r') as fr:
    #    for line in fr.readlines():
    #        tokens = line.strip().split(' ')
    #        object_types[int(tokens[1])] = tokens[0]
    ...
    for i, video_folder in enumerate(test_videos):
        ...
        object_type = object_types[np.argmax(score)]
        predict_object_label = np.argmax(score)
        object_label = test_videos[video_folder][2][1]
        print '{}: {}/{}, {} vs {}'.format(video_folder, i+1, n_videos, predict_object_label, object_label)
        
        if predict_object_label == object_label:
            num_correct += 1
